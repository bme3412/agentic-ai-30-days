      ...
      concepts: [
        {
          ...
          description: `Consider building a customer support agent. Without checkpointing:
...
| Serverless (Lambda, etc.) | PostgresSaver or Redis-based custom |
| Testing | MemorySaver |`,
...
1. **User-Based**: thread_id = user_id
   - One conversation per user, continues forever
   - Good for personal assistants

2. **Session-Based**: thread_id = f"{user_id}-{session_id}"
   - New conversation each session
   - Good for support tickets, task-based interactions

3. **Task-Based**: thread_id = task_id or workflow_id
   - Tied to a specific workflow instance
   - Good for document processing, approvals

4. **Composite**: thread_id = f"{user_id}-{context}-{timestamp}"
...
1. **Debugging**: "Why did the agent do that?" Walk through history to see state at each node.
2. **Auditing**: Compliance requirements often need full interaction logs—checkpoint history provides this.
3. **Analytics**: Track token usage, iteration counts, error rates by analyzing checkpoint metadata.
4. **Quality Assurance**: Review random samples of conversations by loading historical checkpoints.`,
...
**1. Design State for Interruption**
\`\`\`python
class ApprovalState(TypedDict):
    messages: Annotated[list, add_messages]
    draft: str
...
**5. Collect Human Input (Your App's Responsibility)**
...
- Agent can wait hours or days for human response
- Server can restart—state is persisted
- Multiple approval rounds work automatically (revise loops back)
- Full audit trail via checkpoint history`,
...
def recall_node(state):
    relevant = vector_store.search(state["current_query"],
...
def maybe_summarize(state):
    if len(state["messages"]) > 20:
        summary = llm.invoke("Summarize this conversation: " + format_messages(state["messages"]))
...
1. Short-term (checkpointing): Current conversation flow
2. Long-term (external DB): User preferences, past learnings, domain knowledge`,
