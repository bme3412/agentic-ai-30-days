...
# and wait for human input before proceeding`
    ...
    router -->|path A| crewA[Crew A]
    router -->|path B| crewB[Crew B]
    router -->|path C| crewC[Crew C]

    style start fill:#ff5a1f,color:#fff
    style router fill:#ff5a1f,color:#fff`
    },
...
analyst = Agent(
    ...
    style start fill:#ff5a1f,color:#fff
    style router fill:#ff5a1f,color:#fff
    style join fill:#00d084,color:#000`,
        caption: "Start entry, Listen reactions, Router branching, Join waits for parallel paths"
      }
    ],
    keyTakeaways: [
